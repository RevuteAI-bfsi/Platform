class GeminiService {
  ...
  // Analyze speaking transcript and provide feedback
  // In Geminiservice.js
async evaluateAgainstStandard({ questionText, transcript, expectedPoints }) {
  const promptText = `
...
Standard answer points the user should cover:
${expectedPoints.map((pt, i) => `${i + 1}. ${pt}`).join("\n")}

TASK:
1. Identify which of the above points the user DID mention.
2. Identify which are MISSING.
3. Compute an accuracy percentage: (matched points ÷ total points) × 100.
4. Provide a short constructive feedback summary highlighting strengths and areas to improve.

Please respond in strict JSON format:
{
  "matchedPoints": [ /* list of strings exactly as in standard list */ ],
...
`.trim();
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.error?.message?.includes("API key expired")) {
          console.error(
            "API key has expired. Please update the API key in GeminiService.js"
          );
  ...
  getMockResponse() {
    return {
      ...
      total_fillers: 3,
      feedback:
        "Good speaking with minimal filler words. Try to reduce the use of 'um' and 'like'.",
    };
  }
}
